...
In this lab, you should take the time to practice using git and github. This will work best if you for a group of 2, so that you can get experience collaborating with someone on a project through git. Firstly, make sure you know what the following bash commands do. Fire up the bash shell and try them out. There are lots more, but these are the most common ones I use. Google them and try them out!
...
Also from the bash shell, you can use TAB to complete commands and paths and / to seperate paths.

Setting up git
- I recommend you install [git for Windows](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git) rather than the github client. You can optionally install [SourceTree](https://www.sourcetreeapp.com/) which is a nice git gui program that allows you to view exactly what has changes between commits amongst lots of other cool stuff.
...
Basic stuff
- Create the project in Processing and save it somewhere
...
- Create the project on github. Dont forget to create a .gitignore and a readme. 
- Get the url of the new repo and type:
	- ```git remote add origin THE_URL_OF_YOUR_REPO```
- Type ```git pull origin master`` to get the changes from the server
...
- Type ```git push``` to commit your changes locally
- Type ```git push``` to send your changes to the server

Rolling back to a previous commit
- Make a few more commits and push them
- Type ```git log``` to see the history of your branch and get the 40 digit id's of your commits
...
- If you want to go back to an old version of a project and make changes, you will often see articles on stack overflow etc saying that you should hard reset the head. I recommend that you dont do this as this will delete the subsequent commits on the branch and you might want to get these changes back sometime. Instead the best thing to do is make the changes on a new branch so you dont affect the later commits on branch.  
- Now checkout one of the old commits again. Make some changes to the sketch
...
- Now switch back to the master branch by typing ```git checkout master```. Open your sketch and verify that you are at the matest version of the project on the master branch.
...
Deleting files
- git will normally just store files that have been added or modified in a commit. Files that are deleted dont get deleted in the commit so that if you checkout that commit, the files that you deleted will reappear. If you have deleted files in a commit you should use ```git add . --all```. Try it!
...
- Make some changes on to your Processing sketch and save them
...
Congratulations! You are now a git boss.
